...
def muCov(Data):
    """Given Data, return its mean mu and its covariance Matrix"""
    mu = mCol(Data.mean(1))
...
def PCA(Data, dim):
    ...
    eigVal, eigVect = np.linalg.eigh(Covariance)
...
def logPDF_Gau_ND(X, mu, Covariance):
    ...
    logDetCov = np.linalg.slogdet(Covariance)[1]
...
def applyMVG(trainMuCov, testData, priorProb):
    ...
    for id, prob in enumerate(priorProb):
        logScore.append(
            logPDF_Gau_ND(testData, trainMuCov[id][0], trainMuCov[id][1])
            + logPriorProb[id]
        )
...
def k_fold(Data, Label, priorProb, function, k=5, seed=0):
    ...
    for i in range(k):
        ...
        accuracy += function(
            trainData, trainLabel, testData, testLabel, priorPCProb
        ).mean()
...
def EM(X, gmm, psi):
    ...
    while loss_old is None or loss_new - loss_old > limit:
        ...
        for idx in range(len(gmm)):
            S_j[idx, :] = logPDF_Gau_ND(X, gmm[idx][1], gmm[idx][2]) + np.log(
                gmm[idx][0]
            )
        ...
        gmm_new = [
            ((w_new[idx]), vcol(mu_new[:, idx]), C_new[:, :, idx])
            for idx in range(len(gmm))
        ]
