...
def k_fold_cross_validation(D, L, classifier, k, pi, Cfp, Cfn, l, pi_T, seed = 0, preprocessor=None, just_llr = False):
...
if __name__ == "__main__":
    ...
    for LR_type in ["linear", "quadratic"]:
        ...
        with open(fileName, "w") as f:
            ...
            DCF_kfold_raw = []
            for l in l_val:
                minDCF, _ = k_fold_cross_validation(D, L, linear_or_quadratic, 5, 0.5, 10, 1, l, pi_T, seed = 0)
                DCF_kfold_raw.append(minDCF)
                f.write(f" L-val: {l} {minDCF}")
            ...
            DCF_kfold_z = []
            for l in l_val:
                minDCF, _ = k_fold_cross_validation(DN, L, linear_or_quadratic, 5, 0.5, 10, 1, l, pi_T, seed = 0)
                ...
                for i in range(5, 11):
                    ...
                    values.append(
                        k_fold_cross_validation(
                            D, L, linear_or_quadratic, 5, 0.5, 10, 1, l,pi_T,seed=0, preprocessor=preprocessor
                        )
                    )
                    f.write(f"l_val:{l} PCA:{i} {values[-1]}")
